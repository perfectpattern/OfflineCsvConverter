<template>
  <svg viewBox="0 0 401.998 401.998">
    <path
      :class="'fill-current ' + getClasses(sorting, 'up')"
    ...
    />
    <path
      :class="'fill-current ' + getClasses(sorting, 'down')"
...
export default {
  ...
  methods: {
    getClasses(sorting, type) {
      if (sorting === "none") return "sorting-inactive";
      if (
        (sorting === "asc" && type === "up") ||
        (sorting === "desc" && type === "down")
      )
        return "active";
